...
<script type="text/javascript">
	function del(){
		...
		if(res){
			location.href='delMem.do?mid=${seUser}';
		} else {
			return false;
		}
	}
...
<a href="main.do" title="메인으로 이동!"><img alt="logo" src="img/signboard.png" class="logo left"></a><br>
...
<div class="left">
	...
	<c:forEach var="v" items="${datas}">
		...
		<h3>[${m.mid}😎] ${m.content} &emsp;▶&emsp;[좋아요 ${m.favcnt} | 댓글 ${m.comcnt} | ${m.pdate}]
			<c:if test="${!empty seUser}">
				<a href="addFav.do?pnum=${m.pnum}&cnt=${cnt}" title="좋아요🧡">🧡</a>
			</c:if>
	...
	<c:if test="${cnt<PostNum}">
		<a href="main.do?cnt=${cnt+1}&selUser=${selUser}" class="bold">⭐see more...</a>
	</c:if>
	<hr>
	<!-- NEW POST 부분!!! -->
	<mytag:new type="post"/>
	...
	<h3>신규회원 목록</h3>
		...
		<c:forEach var="v" items="${newUsers}">
			<li><a href="main.do?cnt=${cnt}&selUser=${v.mid}">${v.name}</a>님</li>
		</c:forEach>
		</ul>
</div>
<div class="right">
